...
class KindleController extends Controller
{
  ...
  {
    if (!$kindle->books->contains($book)) {
      $kindle->books()->attach($book);
    }

    return redirect()->route('kindles.books', $kindle);
  }
  ...
  public function books(Kindle $kindle)
  {
    $books = $kindle->books;
    return view('kindles.books', compact('kindle', 'books'));
  }
}
